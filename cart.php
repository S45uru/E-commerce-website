...
if (isset($_POST['update_cart'])){
    ...
    $update_qty=$conn->prepare("UPDATE cartt SET qty=? WHERE id=? ");
...
if (isset($_POST['delete_item'])) {
    ...
    $varify_delete_items = $conn->prepare("SELECT * FROM cartt WHERE id=?");
    $varify_delete_items->execute([$cart_id]);

    if ($varify_delete_items->rowCount() > 0) {
        $delete_cart_id = $conn->prepare("DELETE FROM cartt WHERE id=?");
...
if(isset($_POST['empty_cart'])){
    $varify_empty_item = $conn->prepare("SELECT * FROM cartt WHERE user_id=?");
    $varify_empty_item->execute([$user_id]);

    if($varify_empty_item->rowCount()>0){
        $delete_cart_id = $conn->prepare("DELETE FROM cartt WHERE user_id=?");
...
<div class="main">
    ...
    <div class="title2">
        <a href="head.php">Home</a><span> / CART</span>
    </div>
